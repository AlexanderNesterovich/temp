...
<body>
	<%
		...
		user_message = (String) jsp_parameters.get("current_action_result_label");
	...
	<table align="center" border="1" width="90%">
		...
		%>
		<tr>
			<td colspan="6" align="center"><span style="color: red"><%=user_message%></span></td>
		</tr>
		<%
			}
		%>
		...
		%>
		<tr>
			<td colspan="6" align="center"><span style="color: red"><%=inside_message%></span></td>
		</tr>
		<%
			}
		%>
		...
		<tr>
			<td colspan="6" align="center"><a
				href="<%=request.getContextPath()%>/?action=add">Добавить запись</a></td>
		</tr>
		<tr>
 ...
 %>
			...
			<td><a
				href="<%=request.getContextPath()%>/?action=edit&id=<%=person.getId()%>">Редактировать</a></td>
			<td><a
				href="<%=request.getContextPath()%>/?action=delete&id=<%=person.getId()%>">Удалить</a></td>
		</tr>
		<%
			}
		%>
